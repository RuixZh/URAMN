...
class embedder:
    def __init__(self, args):
        ...
        adj, features, labels, idx_train, idx_val, idx_test, truth = process.load_data2(args)

        args.nb_nodes = features.shape[0]
        args.ft_size = features.shape[1]
        args.nb_classes = labels.shape[1]
        args.nb_graphs = len(adj)

        self.neighbor_adj = [process.row_normalize_adj(adj_) for adj_ in adj]

        self.adj = [process.sparse_mx_to_torch_sparse_tensor(adj_) for adj_ in adj]

        self.features = torch.FloatTensor(features.todense())
        if args.isSemi:
            self.train_y = truth[idx_train]
            self.test_y = truth[idx_test]
